...
<div class="container mx-auto px-4 relative">
    ...
    <div class="text-center mb-10 relative">
        <div class="inline-block relative">
            ...
            <div class="w-full h-1 bg-gradient-to-r from-primary-600 via-primary-500 to-primary-600 absolute -bottom-3 left-0"></div>
            <div class="w-3 h-3 bg-primary-600 absolute -bottom-4 left-1/2 transform -translate-x-1/2 rotate-45"></div>
        </div>
        @if($subtitle)
            ...
            <div class="grid md:grid-cols-5 gap-6 items-center">
                <div class="md:col-span-3 relative overflow-hidden rounded-lg shadow-lg">
                    ...
                    <div class="relative h-80 overflow-hidden">
                        <div class="absolute inset-0 bg-gradient-to-r from-primary-600/70 to-transparent z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                        <img src="{{ asset('storage/' . $featuredPost->thumbnail) }}"
                             alt="{{ $featuredPost->title }}"
                             class="w-full h-full object-cover transform transition-transform duration-700 group-hover:scale-105"
                             loading="lazy">
                        <div class="absolute top-4 left-4 z-20">
                            <span class="bg-primary-600 text-white text-xs px-3 py-1.5 rounded-full font-medium tracking-wide">Tin mới nhất</span>
                        </div>
                    </div>
                    @else
                    <div class="bg-gradient-to-r from-primary-500 to-primary-700 h-80 flex items-center justify-center">
                ...
                <div class="md:col-span-2 p-3 md:p-6">
                    <div class="flex items-center mb-3">
                        <span class="inline-flex items-center text-xs bg-primary-50 text-primary-600 py-1 px-2 rounded-full">
                    ...
                    <a href="{{ route('posts.show', $featuredPost->slug) }}" class="block group">
                        <h3 class="text-2xl font-bold text-gray-900 mb-3 group-hover:text-primary-600 transition-colors line-clamp-2">{{ $featuredPost->title }}</h3>
                    </a>
                    @if($featuredPost->seo_description)
                    <p class="text-gray-600 mb-6 line-clamp-3">{{ $featuredPost->seo_description }}</p>
                    @endif
                    <a href="{{ route('posts.show', $featuredPost->slug) }}" class="inline-flex items-center text-primary-600 hover:text-primary-700 font-medium border-b-2 border-primary-600/30 hover:border-primary-600 pb-0.5 transition-colors group">
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow group hover:translate-y-[-4px] duration-300">
                <a href="{{ route('posts.show', $post->slug) }}" class="block">
                    <div class="h-48 overflow-hidden relative">
                        ...
                        @else
                        <div class="w-full h-full bg-gradient-to-r from-primary-100 to-primary-50 flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-primary-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="p-5 border-t border-gray-50">
                    ...
                    <a href="{{ route('posts.show', $post->slug) }}">
                        <h3 class="text-lg font-semibold text-gray-900 mb-2 group-hover:text-primary-600 transition-colors line-clamp-2">{{ $post->title }}</h3>
                    </a>
                    @if($post->seo_description)
                    <p class="text-gray-600 mb-4 line-clamp-2">{{ Str::limit($post->seo_description, 100) }}</p>
                    @endif
                    <div class="flex justify-between items-center pt-2">
                        <a href="{{ route('posts.show', $post->slug) }}" class="inline-flex items-center text-primary-600 hover:text-primary-700 font-medium text-sm group">
                            <span>Đọc tiếp</span>
        ...
        <div class="mb-6 bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100">
            <div class="relative">
                ...
                <div class="h-52 overflow-hidden">
                    <img src="{{ asset('storage/' . $featuredPost->thumbnail) }}"
                         ...
                         loading="lazy">
                    <div class="absolute top-0 right-0 bg-primary-600 text-white text-xs px-3 py-1 rounded-bl-lg font-medium">Nổi bật</div>
                </div>
                @else
                <div class="h-52 bg-gradient-to-r from-primary-500 to-primary-700 flex items-center justify-center">
            ...
            <div class="p-4">
                <div class="flex flex-wrap items-center mb-2 text-xs gap-2">
                    <span class="inline-flex items-center bg-primary-50 text-primary-600 py-1 px-2 rounded-full">
                ...
                @endif
                <a href="{{ route('posts.show', $featuredPost->slug) }}" class="flex justify-between items-center pt-2 text-primary-600 border-t border-gray-100">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-sm border border-gray-100">
                <a href="{{ route('posts.show', $post->slug) }}" class="block">
                    <div class="h-32 overflow-hidden">
                        ...
                        <div class="w-full h-full bg-primary-50 flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-primary-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                            </svg>
                        </div>
                        @endif
                ...
                <div class="p-3">
                    ...
                    <a href="{{ route('posts.show', $post->slug) }}" class="inline-flex items-center text-xs text-primary-600 font-medium">
                        <span>Đọc tiếp</span>
    ...
    <div class="text-center mt-10">
        <a href="{{ $viewAllLink }}" class="inline-block px-6 md:px-8 py-3 md:py-3.5 bg-white text-primary-600 font-medium rounded-full shadow-sm hover:shadow-md border border-primary-600 hover:bg-primary-600 hover:text-white transition-all duration-300">
