@php
    $badge = $data['badge'] ?? 'VỀ CHÚNG TÔI';
    $title = $data['title'] ?? 'ESAT';
    ...
    for ($i = 1; $i <= 4; $i++) {
        $services[] = [
            'image' => $data["feature_{$i}_image"] ?? null,
                    ...
                    <div class="relative overflow-hidden rounded-xl bg-gradient-to-br from-red-50 to-red-100 shadow-lg transform hover:scale-[1.02] transition-all duration-500 p-4 sm:p-6 lg:p-8">
                        <div class="flex flex-col items-center text-center h-full">
                            ...
                            <div class="flex-1 flex flex-col justify-between min-h-0">
                                ...
                                <h3 class="text-sm sm:text-base lg:text-lg font-semibold text-gray-800 mb-2 leading-tight {{ strlen($service['title'] ?? '') > 20 ? 'line-clamp-2' : '' }}"
                                    title="{{ $service['title'] }}">
                                    {{ $service['title'] }}
                                </h3>

                                <!-- Description with responsive text and truncation -->
                                <p class="text-xs sm:text-sm text-gray-600 leading-relaxed {{ strlen($service['desc'] ?? '') > 50 ? 'line-clamp-3' : '' }}"
                                   title="{{ $service['desc'] }}">
                                    {{ $service['desc'] }}
                                </p>
                            </div>
