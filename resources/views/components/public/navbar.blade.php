<header class="bg-white dark:bg-gray-900 shadow-lg sticky top-0 z-50 border-b border-primary-100 dark:border-gray-700 backdrop-blur-md bg-white/95 dark:bg-gray-900/95">
    <!-- Main Navigation -->
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="hidden lg:block flex-1 max-w-2xl mx-8" x-data="searchSuggestions()" @click.away="open = false">
                <form action="{{ route('products.categories') }}" method="GET" class="relative group">
                    ...
                    <input type="text" name="search" placeholder="Tìm kiếm sản phẩm, bài viết..."
                        class="w-full py-3 pl-12 pr-20 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white dark:bg-gray-800 dark:border-gray-600 dark:text-white dark:placeholder-gray-400 shadow-sm hover:shadow-md transition-all duration-200 text-sm"
                        x-model="query"
                        @input.debounce.300ms="search()"
                        @focus="if(query.length >= 2) open = true"
                        autocomplete="off">
                    <button type="submit" class="absolute right-2 top-1/2 transform -translate-y-1/2 bg-gradient-to-r from-primary-600 to-primary-700 text-white rounded-lg px-3 py-1.5 hover:bg-primary-800 transition-all duration-200 shadow-sm hover:shadow-md text-sm font-medium">
                        Tìm
                    </button>
                                ...
                                <template x-for="item in products" :key="'p-'+item.id">
                                    <a :href="item.url" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors border-b border-gray-100 dark:border-gray-700 last:border-0">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-xs text-primary-600 dark:text-primary-400 font-semibold" x-text="item.price ? new Intl.NumberFormat('vi-VN').format(item.price) + ' đ' : 'Liên hệ'"></p>
                                        </div>
            ...
            <div class="hidden lg:flex items-center space-x-3">
                @if(isset($settings) && !empty($settings) && $settings->hotline)
                    <a href="tel:{{ $settings->hotline }}" class="flex items-center px-5 py-2.5 text-primary-600 bg-primary-50 rounded-full hover:bg-primary-100 transition-all duration-200 shadow-sm hover:shadow-md group">
            ...
            <div class="lg:hidden flex items-center gap-3">
                ...
                <button type="button" class="p-2.5 text-gray-600 dark:text-gray-300 hover:text-primary-600 dark:hover:text-primary-400 transition-all duration-200 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Tìm kiếm" id="mobile-search-button">
                ...
                @if(isset($settings) && !empty($settings) && $settings->hotline)
                    <a href="tel:{{ $settings->hotline }}" class="p-2.5 text-primary-600 bg-primary-50 dark:bg-primary-900/20 dark:text-primary-400 rounded-full hover:bg-primary-100 dark:hover:bg-primary-900/30 transition-colors shadow-sm" aria-label="Gọi điện">
            ...
            <div class="p-4 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700" x-data="searchSuggestionsMobile()">
                <form action="{{ route('products.categories') }}" method="GET" class="relative">
                    <input type="text" name="search" id="mobile-search-input" placeholder="Tìm kiếm sản phẩm, bài viết..."
                        class="w-full py-3 px-4 pr-12 border border-gray-200 rounded-full focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-800 dark:border-gray-600 dark:text-white dark:placeholder-gray-400 shadow-sm transition-all duration-200"
                        x-model="query"
                        @input.debounce.300ms="search()"
                        @focus="if(query.length >= 2) open = true"
                        autocomplete="off">
                    <button type="submit" class="absolute right-2 top-1/2 transform -translate-y-1/2 bg-primary-600 hover:bg-primary-700 text-white rounded-full p-2 transition-colors duration-200 shadow-sm">
                        ...
                        <div>
                            ...
                            <template x-for="item in products" :key="'mp-'+item.id">
                                <a :href="item.url" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-100 dark:border-gray-700 last:border-0">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-medium text-gray-900 dark:text-white truncate" x-text="item.name"></p>
                                        <p class="text-xs text-primary-600 font-semibold" x-text="item.price ? new Intl.NumberFormat('vi-VN').format(item.price) + ' đ' : 'Liên hệ'"></p>
                                    </div>
                                </a>
                            </template>
                        </div>
    ...
    <div class="bg-primary-600 dark:bg-primary-700 border-t border-primary-500 dark:border-primary-600 hidden lg:block">
