@php
    ...
    // Sử dụng dữ liệu từ ViewServiceProvider (4 bài viết tin tức nổi bật)
    $newsPostsData = collect();
    if ($isVisible) {
        $newsPostsData = isset($newsPosts) && !empty($newsPosts) ? $newsPosts : collect();
    }
    ...
    <div class="text-center mb-10 relative">
        ...
        <div class="hidden md:block">
            ...
            @if($remainingPosts->count() > 0)
                @if($remainingPosts->count() <= 4)
                    <!-- Grid layout cho 1-4 bài viết còn lại -->
        ...
        @if($postsCount > 0)
            <div class="text-center mt-10">
                <a href="{{ route('posts.index') }}" class="inline-block px-6 md:px-8 py-3 md:py-3.5 bg-white text-red-600 font-medium rounded-full shadow-sm hover:shadow-md border border-red-600 hover:bg-red-600 hover:text-white transition-all duration-300">
